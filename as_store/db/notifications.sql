...
INSERT INTO notification_templates (key, name, category, title_en, body_en, deep_link, channels) VALUES
  ('order_received',  'Order received',       'order',
   ...
   'Thanks {{name}}! We''ve received your order of {{itemCount}} item(s) ({{total}}) and will confirm it shortly.',
   '/orders/{{orderId}}', '["inapp","push"]'::jsonb),
  ('order_confirmed', 'Order confirmed',      'order',
   'Order #{{orderId}} confirmed',
   'Your order is confirmed and being prepared. We''ll let you know when it ships.',
   '/orders/{{orderId}}', '["inapp","push"]'::jsonb),
  ...
  ('order_delivered', 'Order delivered',      'order',
   ...
   'Your order has been delivered. Enjoy! Tap to view your order.',
   '/orders/{{orderId}}', '["inapp","push"]'::jsonb),
  ...
  ('payment_paid',    'Payment received',     'order',
   ...
   'We''ve received your payment of {{total}}. Your order is confirmed.',
   '/orders/{{orderId}}', '["inapp","push"]'::jsonb),
  ('payment_failed',  'Payment failed',       'order',
   'Payment problem on order #{{orderId}}',
   'Your online payment didn''t go through. You can retry from your order page.',
   '/orders/{{orderId}}', '["inapp","push"]'::jsonb),
  ('delivery_feedback', 'Post-delivery feedback', 'survey',
   'How was your order?',
   'Your order #{{orderId}} arrived — tell us how we did in 30 seconds.',
