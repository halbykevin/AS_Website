    ...
    >
      <div className="shell">
        <Reveal>
          {heading && (
            <h2
              className={`text-4xl font-semibold tracking-apple sm:text-5xl ${onDark ? "text-white" : "text-as-ink"}`}
            >
              {heading}
            </h2>
          )}
          {subheading && (
            <p
              className={`mx-auto mt-3 max-w-lg text-lg ${onDark ? "text-white/60" : "text-as-ink/60"}`}
            >
              {subheading}
            </p>
          )}
...
function RichTextSection({ section }) {
  ...
  const onDark = textTheme === "dark";
  const align =
    section.settings?.align === "left" ? "text-left" : "text-center";
    ...
    <section className="py-20" style={{ backgroundColor: bg || "#ffffff" }}>
      <div className={`mx-auto w-full max-w-[760px] px-6 ${align}`}>
        <Reveal>
          {heading && (
            <h2
              className={`text-3xl font-semibold tracking-apple sm:text-4xl ${onDark ? "text-white" : "text-as-ink"}`}
            >
              {heading}
            </h2>
          )}
          {paragraphs.length > 0 && (
            <div
              className={`mt-6 space-y-4 text-lg leading-relaxed ${onDark ? "text-white/70" : "text-as-ink/70"}`}
            >
              {paragraphs.map((p, i) => (
