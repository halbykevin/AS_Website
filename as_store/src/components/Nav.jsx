...
// Apple-style global nav: slim, translucent-dark, blurred. Evenly spaced minimal
// links on desktop; a full-screen blurred overlay menu on mobile.
    ...
    <header className="fixed inset-x-0 top-0 z-50">
      <div className="border-b border-white/10 bg-black/80 backdrop-blur-xl backdrop-saturate-150">
        <nav className="shell-wide flex h-12 items-center justify-between">
          <a href="/" className="flex items-center" aria-label="AS Store">
          ...
          <ul className="hidden items-center gap-7 lg:flex">
            {navLinks.map((l) => (
              <li key={l}>
                <a href="#" className="text-[12px] text-white/80 transition-colors hover:text-white">
                  {l}
                </a>
              </li>
            ))}
