...
export function Field({ label, children, hint }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-sm font-medium text-as-ink/70">
        {label}
      </span>
      {children}
      {hint && (
    ...
    >
      {logo ? (
        ...
        <img
          src={logo}
          alt=""
          className="h-5 w-auto max-w-[96px] shrink-0 object-contain"
        />
      ) : (
...
export function OrDivider({ label = "or" }) {
  return (
    <div className="flex items-center gap-4">
      ...
      <span className="text-xs font-medium uppercase tracking-wider text-as-ink/40">
        {label}
      </span>
      <span className="h-px flex-1 bg-as-ink/10" />
    </div>
  ...
  return (
    <section className="bg-white pb-24 pt-28 sm:pt-32">
      <div className="mx-auto w-full max-w-md px-6">
        <h1 className="text-center text-3xl font-semibold tracking-apple text-as-ink sm:text-4xl">
          {title}
        </h1>
        {subtitle && (
          <p className="mt-2 text-center text-as-ink/55">{subtitle}</p>
        )}
        <div className="mt-8">{children}</div>
        {footer && (
          <div className="mt-6 text-center text-sm text-as-ink/55">
            {footer}
          </div>
        )}
      </div>
