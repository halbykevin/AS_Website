...
// Landing spot for a sign-in that left the site — Google's and Apple's. The API
// sends the shopper here with the token in the URL *fragment* (#token=…), which
// never reaches a server and so stays out of referrers and access logs. We take
// it, confirm it works, and move on — this page is a blink, not a destination.

import { useEffect, useRef, useState } from 'react'
import { useRouter } from 'next/navigation'
import { useAccount, AFTER_SIGN_IN } from '@/lib/account'

export default function TokenLanding({ provider }) {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.replace(`/login?error=${provider}`)
      return
    }
