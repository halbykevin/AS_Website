...
export const analyticsReady = () => config.enabled && Boolean(config.ga4Id || config.adsConversionId)
...
export function pageView(path) {
  if (typeof window === 'undefined' || !analyticsReady() || !config.ga4Id) return
  window.dataLayer = window.dataLayer || []
...
function adsConversion(label, params) {
  ...
  track('conversion', { send_to: `${config.adsConversionId}/${label}`, ...params })
}
...
const sum = (items) => items.reduce((t, i) => t + (Number(i.price) || 0) * (Number(i.qty) || 1), 0)
...
// confirmation page can never inflate the numbers even if the local guard in
// PurchaseTracker is cleared.
