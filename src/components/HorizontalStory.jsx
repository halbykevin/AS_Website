  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={fill ? { animationDelay: '-2.6s' } : { animationDelay: '-2.6s', aspectRatio: `16 / ${ratio}` }}
      >
        <div
        ...
        {/* Shop now — pinned to the banner's top-right, inset far enough to clear
            the rounded corner. A sibling of the drag container (not a child) so a
            swipe can never fire it. */}
        <ShopNow href={storeHref} />
...
// The banner's own call to action. Same href as the slides, so it follows the
// storeUrl setting (external store, or the in-site coming-soon page if cleared).
function ShopNow({ href }) {
  const external = /^https?:\/\//i.test(href)
  const cls =
